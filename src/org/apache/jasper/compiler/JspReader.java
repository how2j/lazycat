...
class JspReader {
	 ...
	 * file given.
	 */
	public JspReader(JspCompilationContext ctxt, String fname, String encoding, InputStreamReader reader,
	 ...
	 *
	 * @param readAhead
	 *            The number of characters to read ahead. NOTE: This is zero
	 *            based.
	 *
	 ...
	 *         stream is positioned after the search string,
	 ...
	 * initially escaped by a '\' and any EL expressions. When returned, the
	 * context is positioned past the end of the match.
	 *
	 * @param s
	 *            The String to match.
	 * @param ignoreEL
	 *            <code>true</code> if something that looks like EL should not
	 *            be treated as EL.
	 * @return A non-null <code>Mark</code> instance (positioned immediately
	 *         before the search string) if found, <strong>null</strong>
	 *         otherwise.
	...
	private void pushFile(String file, String encoding, InputStreamReader reader) throws JasperException {
		...
		if (fileid == -1) {
			// Bugzilla 37407:
			// http://bz.apache.org/bugzilla/show_bug.cgi?id=37407
			if (reader != null) {
				try {
					reader.close();
				} catch (Exception any) {
