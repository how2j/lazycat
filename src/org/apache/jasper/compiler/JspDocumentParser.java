...
class JspDocumentParser extends DefaultHandler2 implements TagConstants {
	 ...
	 */
	public static Node.Nodes parse(ParserController pc, String path, JarFile jarFile, Node parent, boolean isTagFile,
	...
	public void startElement(String uri, String localName, String qName, Attributes attrs) throws SAXException {
		...
		if (attrs != null) {
			...
			for (int i = attrs.getLength() - 1; i >= 0; i--) {
				...
				if (!attrQName.startsWith("xmlns")) {
					...
					nonTaglibAttrs.addAttribute(attrs.getURI(i), attrs.getLocalName(i), attrs.getQName(i),
		...
		if (tagDependentPending && JSP_URI.equals(uri) && localName.equals(BODY_ACTION)) {
			...
			tagDependentNesting++;
			current = parseStandardAction(qName, localName, nonTaglibAttrs, nonTaglibXmlnsAttrs, taglibAttrs,
					startMark);
			return;
		}
