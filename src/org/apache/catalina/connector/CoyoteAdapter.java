...
public class CoyoteAdapter implements Adapter {
	...
	private static final String POWERED_BY = "Servlet/3.0 JSP/2.2 " + "(" + ServerInfo.getServerInfo() + " Java/"
	...
	public boolean event(org.apache.coyote.Request req, org.apache.coyote.Response res, SocketStatus status) {
		...
		try {
			...
			if (!error && !response.isClosed() && (request.getAttribute(RequestDispatcher.ERROR_EXCEPTION) != null)) {
				...
				request.getEvent().setEventSubType(null);
				error = true;
				connector.getService().getContainer().getPipeline().getFirst().event(request, response,
						request.getEvent());
			}
			...
			if (error || response.isClosed() || !request.isComet()) {
				((Context) request.getMappingData().context).logAccess(request, response,
