...
public abstract class AbstractReplicatedMap<K, V>
	...
	protected void init(MapOwner owner, Channel channel, String mapContextName, long timeout, int channelSendOptions,
		...
		if (log.isInfoEnabled())
			log.info("AbstractReplicatedMap[" + mapContextName + "] initialization was completed in " + complete
					+ " ms.");
	}
		...
		synchronized (mapMembers) {
			...
			for (Member member : members) {
				...
				if ((now - access) > timeout) {
					log.warn(
							"Member[" + member + "] in the Map[" + mapname + "] has timed-out in the ping processing.");
	...
	public void transferState() {
		try {
			...
			if (backup != null) {
				MapMessage msg = new MapMessage(mapContextName, getStateMessageType(), false, null, null, null, null,
						null);
				...
				while (i.hasNext()) {
					...
					if (entry != null && entry.isSerializable()) {
						...
						MapMessage me = new MapMessage(mapContextName,
								copy ? MapMessage.MSG_COPY : MapMessage.MSG_PROXY, false, (Serializable) entry.getKey(),
								copy ? (Serializable) entry.getValue() : null, null, entry.getPrimary(),
	...
	public void leftOver(Serializable msg, Member sender) {
		...
		try {
			...
			if (mapmsg.getMsgType() == MapMessage.MSG_START) {
				...
				if (log.isInfoEnabled())
					log.info("PING message has been received beyond the timeout period. " + "The map member[" + member
	...
	protected void printMap(String header) {
		try {
			System.out.println("\nDEBUG MAP:" + header);
			System.out
					.println("Map[" + new String(mapContextName, CHARSET_ISO_8859_1) + ", Map Size:" + innerMap.size());
	...
	public static class MapMessage implements Serializable {
		...
		@Override
		public Object clone() {
			MapMessage msg = new MapMessage(this.mapId, this.msgtype, this.diff, this.key, this.value, this.diffvalue,
