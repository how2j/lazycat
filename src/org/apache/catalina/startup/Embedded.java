...
public class Embedded extends StandardService {
	...
	public void setRedirectStreams(boolean redirectStreams) {
		...
		support.firePropertyChange("redirectStreams", Boolean.valueOf(oldRedirectStreams),
	...
	public Connector createConnector(String address, int port, String protocol) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Creating connector for address='" + ((address == null) ? "ALL" : address) + "' port='" + port
					+ "' protocol='" + protocol + "'");
		}
	...
	protected void initDirs() {
		...
		if (catalinaHome == null) {
			...
			if (j2eeHome != null) {
				...
				catalinaHome = IntrospectionUtils.guessInstall(Globals.CATALINA_HOME_PROP, Globals.CATALINA_BASE_PROP,
						"catalina.jar");
				if (catalinaHome == null) {
					catalinaHome = IntrospectionUtils.guessInstall("tomcat.install", Globals.CATALINA_HOME_PROP,
							"tomcat.jar");
				}
			}
		}
