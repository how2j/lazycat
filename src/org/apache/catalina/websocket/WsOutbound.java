...
public class WsOutbound {
	...
	public WsOutbound(UpgradeOutbound upgradeOutbound, StreamInbound streamInbound, int byteBufferSize,
			int charBufferSize) {
	...
	private boolean validateCloseStatus(int status) {
		...
		if (status == Constants.STATUS_CLOSE_NORMAL || status == Constants.STATUS_SHUTDOWN
				|| status == Constants.STATUS_PROTOCOL_ERROR || status == Constants.STATUS_UNEXPECTED_DATA_TYPE
				|| status == Constants.STATUS_BAD_DATA || status == Constants.STATUS_POLICY_VIOLATION
				|| status == Constants.STATUS_MESSAGE_TOO_LARGE || status == Constants.STATUS_REQUIRED_EXTENSION
				|| status == Constants.STATUS_UNEXPECTED_CONDITION || (status > 2999 && status < 5000)) {
