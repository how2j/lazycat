...
public interface SessionMessage extends ClusterMessage {
	 ...
	 * expire on the network
	 */
	public static final int EVT_SESSION_ACCESSED = 3;
	/**
	 * Event type used when a server comes online for the first time. The first
	 * thing the newly started server wants to do is to grab the all the
	 * sessions from one of the nodes and keep the same state in there
	 */
	public static final int EVT_GET_ALL_SESSIONS = 4;
	/**
	 * Event type used when an attribute has been added to a session, the
	 * attribute will be sent to all the other nodes in the cluster
	 */
	...
	/**
	 * Event type used when context manager doesn't exist. This is used when the
	 * manager which send a session state does not exist.
	 */
