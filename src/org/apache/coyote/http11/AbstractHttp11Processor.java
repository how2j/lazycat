...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
	 ...
	 */
	protected void initializeFilters(int maxTrailerSize, Set<String> allowedTrailerHeaders, int maxExtensionSize,
			int maxSwallowSize) {
	...
	private void addInputFilter(InputFilter[] inputFilters, String encodingName) {
		...
		if (encodingName.equals("identity")) {
			...
			if (getLog().isDebugEnabled()) {
				getLog().debug(sm.getString("http11processor.request.prepare") + " Unsupported transfer encoding ["
						+ encodingName + "]");
			}
		}
	}
	...
	protected void prepareRequest() {
		...
		if (protocolMB.equals(Constants.HTTP_11)) {
			...
			setErrorState(ErrorState.CLOSE_CLEAN, null);
			if (getLog().isDebugEnabled()) {
				getLog().debug(sm.getString("http11processor.request.prepare") + " Unsupported HTTP version \""
						+ protocolMB + "\"");
			}
		}
