...
public abstract class AbstractAjpProcessor<S> extends AbstractProcessor<S> {
	...
	/**
	 * Send AJP flush packet when flushing. An flush packet is a zero byte AJP13
	 * SEND_BODY_CHUNK packet. mod_jk and mod_proxy_ajp interprete this as a
	 * request to flush data to the client. AJP always does flush at the and of
	 * the response, so if it is not important, that the packets get streamed up
	 * to the client, do not use extra flush packets. For compatibility and to
	 * stay on the safe side, flush packets are enabled by default.
	 */
	...
	public final void action(ActionCode actionCode, Object param) {
		...
		case REQ_HOST_ATTRIBUTE: {
			...
			if (request.remoteHost().isNull()) {
				try {
					request.remoteHost()
							.setString(InetAddress.getByName(request.remoteAddr().toString()).getHostName());
