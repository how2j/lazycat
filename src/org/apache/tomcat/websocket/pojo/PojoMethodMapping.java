...
public class PojoMethodMapping {
	...
	private static class MessageHandlerInfo {
		...
		public MessageHandlerInfo(Method m, List<DecoderEntry> decoderEntries) {
			...
			for (int i = 0; i < types.length; i++) {
				...
				if (String.class.isAssignableFrom(types[i])) {
					if (indexString == -1) {
						...
						throw new IllegalArgumentException(sm.getString("pojoMethodMapping.duplicateMessageParam",
					...
					if (indexBoolean == -1) {
						...
						throw new IllegalArgumentException(sm.getString("pojoMethodMapping.duplicateLastParam",
								m.getName(), m.getDeclaringClass().getName()));
					}
					...
					if (indexSession == -1) {
						...
						throw new IllegalArgumentException(sm.getString("pojoMethodMapping.duplicateSessionParam",
								m.getName(), m.getDeclaringClass().getName()));
					}
					...
					if (indexPong == -1) {
						...
						throw new IllegalArgumentException(sm.getString("pojoMethodMapping.duplicatePongMessageParam",
								m.getName(), m.getDeclaringClass().getName()));
					}
				} else {
			...
			if (indexPong != -1) {
				if (indexPayload != -1) {
					throw new IllegalArgumentException(sm.getString("pojoMethodMapping.pongWithPayload", m.getName(),
			...
			if (indexInputStream != -1 && indexBoolean != -1) {
				throw new IllegalArgumentException(sm.getString("pojoMethodMapping.partialInputStream", m.getName(),
						m.getDeclaringClass().getName()));
			}
