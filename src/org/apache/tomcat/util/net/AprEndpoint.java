...
public class AprEndpoint extends AbstractEndpoint<Long> {
	...
	static {
		/*
		 * Default used if SSLProtocol is not configured, also used if
		 * SSLProtocol="All"
		 */
	 ...
	 * supports send file once it has been initialised. If useSendFile is set by
	...
	public void bind() throws Exception {
		...
		if (isSSLEnabled()) {
			...
			SSLContext.setCertificate(sslContext, SSLCertificateFile, SSLCertificateKeyFile, SSLPassword,
					SSL.SSL_AIDX_RSA);
				...
				try {
					...
					try {
						...
						if (log.isDebugEnabled()) {
							...
							Sockaddr addr = Address.getInfo(sa);
							log.debug(sm.getString("endpoint.apr.remoteport", Long.valueOf(socket),
	...
	public class Poller implements Runnable {
		...
		private void add(long socket, int timeout, int flags) {
			if (log.isDebugEnabled()) {
				String msg = sm.getString("endpoint.debug.pollerAdd", Long.valueOf(socket), Integer.valueOf(timeout),
						...
						if (rv > 0) {
							...
							for (int n = 0; n < rv; n++) {
								...
								if (getLog().isDebugEnabled()) {
									log.debug(sm.getString("endpoint.debug.pollerProcess",
											Long.valueOf(desc[n * 2 + 1]), Long.valueOf(desc[n * 2])));
								}
								...
								if (wrapper.isComet()) {
									...
									if (((desc[n * 2] & Poll.APR_POLLHUP) == Poll.APR_POLLHUP)
										...
										// Unknown event
										getLog().warn(sm.getString("endpoint.apr.pollUnknownEvent",
									...
									// Unknown event
									getLog().warn(
											sm.getString("endpoint.apr.pollUnknownEvent", Long.valueOf(desc[n * 2])));
									// Close socket and clear pool
									closeSocket(desc[n * 2 + 1]);
								}
							}
