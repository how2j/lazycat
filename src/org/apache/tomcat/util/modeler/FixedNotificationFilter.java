 ...
 *
 * This class is immutable - after you construct it it'll filter based on a
 * fixed set of notification names.
 *
 * The JMX specification requires the filters to be called before the
 * notifications are sent. We can call this filter well in advance, when the
 * listener is added. Based on the result we can maintain separate channels for
 * each notification - and reduce the overhead.
 *
...
public class FixedNotificationFilter implements NotificationFilter {
	 ...
	 *
	 * @param names
	 *            Names of the notification types
	 */
