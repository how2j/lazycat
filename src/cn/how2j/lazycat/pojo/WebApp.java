...
import cn.how2j.lazycat.util.LastVisitManager;
...
//		int duration= (int) (( this.getEndTime()-this.getStartTime() )/ 1000);
//		System.out.println(duration);
		
		String pageFormat;
		String url;
		String lastVisit = LastVisitManager.get(this.getId());
		String uri;
		...
		if(null!=lastVisit)
			...
			if (StringUtils.isEmpty(this.getName())) 
				uri = "/"+this.getTestPage();
			else
				uri = "/"+this.getName() + "/"+this.getTestPage();
		}
		
		pageFormat = "http://localhost:%d%s"; 
		
	
		url = String.format(pageFormat, this.getTomcat().getPort(), uri);
